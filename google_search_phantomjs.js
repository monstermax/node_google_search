...
console.log("gg_params", gg_params);
...
if (curl_config.proxy_file) {
	...
	if (FS.exists(curl_config.proxy_file)) {
		//console.error('Reading file://' + curl_config.proxy_file);
		var proxy_contents = FS.read(curl_config.proxy_file).toString();
...
if (gg_params.simulate) {
	console.log("url", gg_url);
	phantom.exit(0);
}

// Parse google result content
getPage(gg_url, curl_config, function (content) {
	parsePage(content, result_types, selected_result_types, parseResultItemGoogle);
});
...
function usage(rc) {
	...
	var usage = [
		...
		'',
		'Usage: $ node google_search_phantomjs.js [<options>] <keyword>',
		'',
		'  Placement options :',
...
function getPage(page_url, curl_config, callback) {
	console.debug('getPage');

	var url_md5 	= md5(page_url);
	...
	console.debug('DEBUG page = new WebPage()');
	...
	if (curl_config.use_cache) {
		...
		if (FS.exists(cache_file)) {
			...
			var html = FS.read(cache_file).toString();
			page.content = html;
	...
	page.settings.userAgent = curl_config.agent;

	page.open(page_url, function (status) {
		console.debug('page.open');
...
function parsePage(page, result_types, selected_result_types, callback){
	console.debug('parsePage');
...
function parsePageTmp(page, result_types, selected_result_types, callback) {
	console.debug('parsePage', page);
	...
	for (var i=0, l=selected_result_types.length; i<l; i++) {
		...
		for (result_type_placement_name in result_type_placements) {
			...
			console.debug('aa');
			var pattern_results = page.evaluate((function (){
				var jpath = this;
				console.debug('page.evaluate', jpath);
			...
			console.debug('bb', pattern_results);
...
if (false){
	...
	page.open(remote_url, function (status) {
		console.debug('DEBUG page.open');

		if (status !== 'success') {
			console.log('Unable to access network');

		} else {
			console.debug('DEBUG page.opened');

			page.includeJs("https://ajax.googleapis.com/ajax/libs/jquery/1.6.2/jquery.min.js", function () {
				console.debug('DEBUG jQuery injected');
